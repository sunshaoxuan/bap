...
import jp.co.bobb.canal.config.Canal2Config;
...
public class Canal2Runner {
     ...
     * 重启
     */
    public static void restart(AmqpTemplate rabbitTemplate, String vipDatabase) {
        Canal2Runner canalRunner = new Canal2Runner();
        canalRunner.rabbitTemplate = rabbitTemplate;
        canalRunner.vipDatabase = vipDatabase;
        canalRunner.starter();
    }
    ...
    private void run() {
        CanalConnector connector = CanalConnectors.newSingleConnector(
                new InetSocketAddress(Canal2Config.host, Canal2Config.port),
                Canal2Config.instance,
                "",
                ""
        );
        try {
            connector.connect();
            connector.subscribe(vipDatabase + ".vip_member");
            connector.rollback();
            ++runners;
            Message message;
            while (true) {
